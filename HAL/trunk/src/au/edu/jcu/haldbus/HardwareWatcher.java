...
/**
 * Objects of this class run a single thread waiting for HAL DeviceAdded/DeviceRemoved signals. When such a signal is received, 
 * a list of {@link HalFilterInterface} objects is matched against the device's properties. If enough 
 ...
 * of interest as contained in the matching {@link HalFilterInterface}.   
...
public class HardwareWatcher implements Runnable, DBusSigHandler{
	 ...
	 * This map is required so that when a DeviceRemoved signal is received, previous successful matches 
	 * can be notified of the DeviceRemoved signal. MatchingFilters are added to this map when a 
	 * DeviceAdded signal is received and the matches in these filters are successful.
	...
	public void addClient(HalFilterInterface f){
		synchronized (clients) {
			if(clients.contains(f))
				throw new AddRemoveElemException("Filter "+f.getName()+" already registered");
			clients.add(f);
		}
	}
	...
	public void removeClient(HalFilterInterface f){
		synchronized (clients) {
			if(!clients.contains(f))
				throw new AddRemoveElemException("Filter "+f.getName()+" not registered");
			clients.remove(f);
		}
	}
	
	/**
	 * This method is called to update the current client filter list with the supplied one.
	 * New clients in the given list are added, and existing clients not in the list are removed. 
	 */
	...
	public void handle(DBusSignal s) {
		if(s instanceof DeviceAdded) {
			...
			try {properties.add(getAllProperties(d.udiAdded));}
				...
				System.out.println("Cant handle newly added device - unable to list its properties");
			}
	...
	private void initialCheck() {
		...
		try {
			...
			System.out.println("Cannot carryout initial check: DBus error "+e.getMessage());
		}
	}
		...
		synchronized(clients) {
			for(HalFilterInterface c: clients){
				...
				if(c.getMinMatches()<=matches.size() && matches.size()<=c.getMaxMatches()) {
					...
					udiMatches.get(getUDI(map)).add(new MatchingFilter(c, matches));
	...
	/**
	 * This method calls {@link Thread#join()} on the thread object and wait till the thread exits,
	 * or an {@link InterruptedException} is caught, and then returns.
	 */
	private void join(){
		try {
			t.join();
