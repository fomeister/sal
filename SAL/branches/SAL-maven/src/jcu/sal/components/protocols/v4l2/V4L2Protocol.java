...
public class V4L2Protocol extends Protocol {
	...
	protected void internal_parseConfig() throws ConfigurationException {
		...
		for (int id = 0; id < v4l2c.length; id++) {
			...
			//getValue command
			i = cmls.addPrivateCMLDesc(V4L2CML.CCD_KEY, GET_CONTROL_METHOD, "get"+name.replace(" ", ""), "Fetches the value of "+name, new String[0], new String[0]);
			ctrls.put(String.valueOf(i), v4l2c[id]);
			
			//setValue command
			i = cmls.addPrivateCMLDesc(V4L2CML.CCD_KEY, SET_CONTROL_METHOD, "set"+name.replace(" ", ""), "Sets the value of "+name, new String[0], new String[0]);
			ctrls.put(String.valueOf(i), v4l2c[id]);
		}
	}
