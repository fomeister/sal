...
public class RMIClientView extends AbstractClientView implements ClientEventHandler, StreamCallback{
	...
	public void connect(String agentIP, String ourIP) throws NotFoundException, RemoteException, ConfigurationException{
		SALAgent a = rmiController.connect(name, agentIP, ourIP);
		rmiController.registerEventHandler(a, this, Constants.PROTOCOL_MANAGER_PRODUCER_ID);
		rmiController.registerEventHandler(a, this, Constants.SENSOR_MANAGER_PRODUCER_ID);
		rmiController.registerEventHandler(a, this, Constants.SENSOR_STATE_PRODUCER_ID);
	}
	...
	public void release(){
		...
		for(SALAgent a : tree.getAgents()){
			try {
				rmiController.unregisterEventHandler(a, this, Constants.PROTOCOL_MANAGER_PRODUCER_ID);
				rmiController.unregisterEventHandler(a, this, Constants.SENSOR_MANAGER_PRODUCER_ID);
				rmiController.unregisterEventHandler(a, this, Constants.SENSOR_STATE_PRODUCER_ID);
			} catch (NotFoundException e1) {
				e1.printStackTrace();
			} 
			rmiController.disconnect(a);
		}
	}
	...
	public void sendCommand(Hashtable<String, String> values,
		...
		new Thread(new Runnable(){
			public void run(){
				Response r;
				try {
					r = rmiController.execute(c.getAgent(), cf.getCommand(), c.getSMLDescription().getID());
	...
	public void handle(Event e, SALAgent a) throws RemoteException {
		...
		if(e instanceof ProtocolListEvent){
			...
			if(ple.getType()==ProtocolListEvent.PROTOCOL_ADDED){
				...
				try {
					ProtocolConfigurations ps = new ProtocolConfigurations(rmiController.listProtocols(a));
			...
			if(sne.getType()==SensorNodeEvent.SENSOR_NODE_ADDED){
				...
				try {
					SMLDescription sml = new SMLDescription(rmiController.listSensor(a, sne.getSourceID()));
