...
import jcu.sal.common.exceptions.JaxbException;
...
public class JaxbHelper {
	...
	private static final String PLUGINS_XML_PACKAGE = "jcu.sal.config.plugins.xml";
	...
	private static void prepareJAXB() throws JAXBException {
		if (context == null) {
			context = JAXBContext.newInstance(PLUGINS_XML_PACKAGE);
	...
	public static <T> T fromXmlString(Class<T> c, String xmlString) throws XmlException {
		try {
			if (xmlString == null) {
			...
			T o = (T) unmarshaller.unmarshal(new StreamSource(new StringReader(xmlString)));
	...
	public static <T> T fromFile(Class<T> c,File f) throws XmlException {
